...
function custom_override_checkout_fields( $checkout ) {
	 echo '<div id="custom_checkout_field"><h3>' . __('Only for special customers:') . '</h3>';
		woocommerce_form_field('pole_myworld', array(
		'type' => 'text',
		'class' => array('my-field-class form-row-wide'),
		'label' => __('Enter your code here'),
		'placeholder' => __('Your ID'),
		),
		$checkout->get_value('pole_myworld'));
	 echo '</div>';
	      //return $fields;
}
// Save field MyWorld in database
...
// Add MyWorld field in administration of order
...
function my_custom_checkout_field_display_admin_order_meta($order){
    ...
    echo '<p><strong>'.__('My coode is').':</strong> ' . $rjedan_value . '</p>';
}
// Add MyWorld field in email
...
function rjedan_email_template($order_obj){
    ...
    echo '<p><strong>My code is: '.$rjedan_email.'</strong></p>';
}
